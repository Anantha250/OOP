...
def show_page(page, current_frame):
    ...
    if page == 11:
        frame_home_member_test.pack(fill="both", expand=True)
...
label = Label(frame_home_member_test, image=bg_pic)
label.place(relheight=1, relwidth=1)

my_frame = scrolledtext.ScrolledText(frame_home_member_test, width=48, height=40)
my_frame.place(relx=0.1, rely=0.5, anchor=tk.CENTER, x=170, y=30)

show_slides(my_frame)
slides_button = ttk.Button(frame_home_member_test, text="Our Slides", command=lambda :show_slides(my_frame), width=24)
slides_button.place(relx=0.1,rely=0.1, anchor=ttk.NW,x=-40,y=10.5)
pool_button = ttk.Button(frame_home_member_test, text="Pool", command=lambda :show_pool(my_frame), width=23)
pool_button.place(relx=0.2,rely=0.1, anchor=ttk.NW,x=-20,y=10.5)
contact = Frame(frame_home_member_test, relief="ridge",borderwidth=20,padx=20,pady=20)
...
email_label = ttk.Label(contact, text="Email : Dkub@email.com", font=font_contact)
email_label.pack(anchor='w')

email_label = ttk.Label(contact, text="Phone number : 0-2329-8106", font=font_contact)
email_label.pack(anchor='w')

hour_label = ttk.Label(contact, text="Office hours : Monday - Sunday: 10:00-18:00", font=font_contact)
hour_label.pack(anchor='w')

home_button = ttk.Button(frame_home_member_test, text="Home", command=lambda : show_page(11,frame_home_member), width=width_button, padding=pading_button)
home_button.place(x=0,y=0)

service_button = ttk.Button(frame_home_member_test, text="Service", command= lambda: show_page(14,frame_home_member), width=width_button, padding=pading_button)
service_button.place(relx=0.2,x=102,y=0)

login_button = ttk.Button(frame_home_member_test, text="info", command=lambda: get_member_info(member_id), width=width_button, padding=pading_button)
login_button.place(relx=0.5,x=12,y=0)

register_button = ttk.Button(frame_home_member_test, text="Log out", command=log_out, width=width_button, padding=pading_button)
register_button.place(relx=0.7,x=102,y=0)
...
home_button = ttk.Button(home_service, text="Home", command=lambda: show_page(11, services_frame), width=width_botton, padding=pading_button)
home_button.pack(side="left", padx=0, pady=10)

login_button = ttk.Button(home_service, text="info", command=lambda: get_member_info_service(member_id), width=width_botton, padding=pading_button)
login_button.pack(side="left", padx=0, pady=10)

register_button = ttk.Button(home_service, text="Log out", command=log_out_service, width=width_botton, padding=pading_button)
...
def wavepool_zone(cabana_zone_frame):
    ...
    wavepool = "/Users/sirima/Documents/Python/OOP/Project_final/wavepool_zone.png"
    wavepool_font_size = 10
    
    create_zone(wavepool_frame, wavepool)
    
    w02 = ttk.Button(wavepool_frame, text="W02", style = get_button_style('standard', wavepool_font_size - 1))
    w02.place( x = 71, y = 545, anchor = CENTER) 
    w03 = ttk.Button(wavepool_frame, text="W03", style = get_button_style('medium', wavepool_font_size))
    w03.place( x = 121, y = 548, anchor = CENTER)
    w06 = ttk.Button(wavepool_frame, text="W06", style = get_button_style('medium', wavepool_font_size))
    w06.place( x = 289, y = 551, anchor = CENTER)
    w07 = ttk.Button(wavepool_frame, text="W07", style = get_button_style('medium', wavepool_font_size))
    w07.place( x = 361, y = 549, anchor = CENTER)
    w08 = ttk.Button(wavepool_frame, text="W08", style = get_button_style('medium', wavepool_font_size))
    w08.place( x = 409, y = 540, anchor = CENTER)
    w11 = ttk.Button(wavepool_frame, text="W11", style = get_button_style('standard', wavepool_font_size - 2))
    w11.place( x = 496, y = 530, anchor = CENTER)
    w13 = ttk.Button(wavepool_frame, text="W13", style = get_button_style('standard', wavepool_font_size - 2))
    w13.place( x = 606, y = 540, anchor = CENTER)
    w15 = ttk.Button(wavepool_frame, text="W15", style = get_button_style('large', wavepool_font_size))
    w15.place( x = 780, y = 391, anchor = CENTER)
    w18 = ttk.Button(wavepool_frame, text="W18", style = get_button_style('standard', wavepool_font_size - 2))
    w18.place( x = 816, y = 185, anchor = CENTER)
    w21 = ttk.Button(wavepool_frame, text="W21", style = get_button_style('standard', wavepool_font_size - 2))
    w21.place( x = 873, y = 233, anchor = CENTER)
    
    cabana_list = [w02, w03, w06, w07, w08, w11, w13, w15, w18, w21]
...
def hill_zone(cabana_zone_frame):
    ...
    h01 = ttk.Button(hill_frame, text="H01", style = get_button_style('standard',hill_font_size))
    h01.place( x = 545, y = 560, anchor = CENTER) 
    h02 = ttk.Button(hill_frame, text="H02", style = get_button_style('standard',hill_font_size))
    h02.place( x = 518, y = 515, anchor = CENTER) 
    h03 = ttk.Button(hill_frame, text="H03", style = get_button_style('standard',hill_font_size))
    h03.place( x = 484, y = 476, anchor = CENTER) 
    ...
    h07 = ttk.Button(hill_frame, text="H07", style = get_button_style('standard',hill_font_size))
    h07.place( x = 229, y = 503, anchor = CENTER)
    ...
    h10 = ttk.Button(hill_frame, text="H10", style = get_button_style('standard',hill_font_size))
    h10.place( x = 153, y = 383, anchor = CENTER)
    h11 = ttk.Button(hill_frame, text="H11", style = get_button_style('medium',hill_font_size))
    h11.place( x = 256, y = 322, anchor = CENTER)
...
def family_zone(cabana_zone_frame):
    ...
    k03 = ttk.Button(family_frame, text="K03", style = get_button_style('medium', 22))
    k03.place(  x = 783, y = 530, anchor = CENTER) 
    k05 = ttk.Button(family_frame, text="K05", style = get_button_style('standard', 18))
    k05.place( x = 645, y = 595, anchor = CENTER) 
    k09 = ttk.Button(family_frame, text="K09", style = get_button_style('medium', 21))
    k09.place(x = 160, y = 460, anchor = CENTER) 
    k10 = ttk.Button(family_frame, text="K10", style = get_button_style('standard', 18))
    k10.place( x = 116, y = 400, anchor = CENTER) 
    k11 = ttk.Button(family_frame, text="K11", style = get_button_style('large', 24))
    k11.place( x = 181, y = 169 , anchor = CENTER) 
    k17 = ttk.Button(family_frame, text="K17", style = get_button_style('medium', 22))
    k17.place( x = 426, y = 460, anchor = CENTER)

    cabana_list = [k03, k05, k09, k10, k11, k17]
...
def login():
        ...
        password_l = password_label_login.get()
        payload = {
            "email": email_l,
            "password": password_l
        }
...
style.configure("TButton",background="darkblue")# กำหนดสีพื้นหลังและสีข้อความให้กับปุ่ม
...
email_entry = ttk.Entry(login_detail_frame)
...
password_entry = ttk.Entry(login_detail_frame, show="*")
...
name_label = Label(register_detail_frame, text="Name:")
name_label.grid(row=row_position, column=0, padx=10, pady=5,sticky='w')

name_entry = Entry(register_detail_frame)
name_entry.grid(row=row_position, column=1, padx=10, pady=5,sticky="news")

email_label = Label(register_detail_frame, text="Email:")
email_label.grid(row=row_position+1, column=0, padx=10, pady=5,sticky='w')

email_entry = Entry(register_detail_frame)
email_entry.grid(row=row_position+1, column=1, padx=10, pady=5,sticky="news")

phone_no_label = Label(register_detail_frame, text="Phone Number:")
phone_no_label.grid(row=row_position+2, column=0, padx=10, pady=5 ,sticky='w')

phone_no_entry = Entry(register_detail_frame)
phone_no_entry.grid(row=row_position+2, column=1, padx=10, pady=5,sticky="news")
...
service_button = ttk.Button(frame_home_member, text="Service", command=lambda: show_page(14,frame_home_member),width=width_botton,padding=pading_button)
...
contact_label = ttk.Label(contact, text="CONTACT",font = ("Cambria", 14, "bold"))
contact_label.grid(row=0, column=0,sticky='w',ipady=10,)
